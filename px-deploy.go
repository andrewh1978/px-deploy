...
*/				
				...
				for i :=1; i <= clusters ; i++ {
					// get associated Security groups (generated by ELB/EKS)
					...
					fmt.Println("Deleting EKS/ELB generated Security Groups:")
					//range security groups
					for _,i := range sg.SecurityGroups {
						fmt.Printf("  %s \n",*i.GroupName)
						var rules_egress []string
						var rules_ingress []string
						//get ingress/egress rules of security group to delete this first
						sgrules,err := client.DescribeSecurityGroupRules(context.TODO(), &ec2.DescribeSecurityGroupRulesInput{
							Filters: []types.Filter {
							{
						...
						if err != nil {
							fmt.Println("Error getting SG Rules:")
							fmt.Println(err)
							return
						}
						for _,z := range sgrules.SecurityGroupRules { 
							
							if *z.IsEgress 	{
								fmt.Printf("    Rule %s (egress)\n", *z.SecurityGroupRuleId)
								rules_egress = append(rules_egress, *z.SecurityGroupRuleId)
							} else {
								fmt.Printf("    Rule %s (ingress)\n", *z.SecurityGroupRuleId)
								rules_ingress = append(rules_ingress, *z.SecurityGroupRuleId)
							}
						}

						if len(rules_ingress) > 0 {

							_,err = client.RevokeSecurityGroupIngress(context.TODO(), &ec2.RevokeSecurityGroupIngressInput{
								//DryRun: aws.Bool(true),
								GroupId: aws.String(*i.GroupId),
								SecurityGroupRuleIds: rules_ingress,
							})
						
							if err != nil {
								fmt.Println("Error deleting ingress rules:")
								fmt.Println(err)
								return
							}
						}
						
						if len(rules_egress) > 0 {
							_,err = client.RevokeSecurityGroupEgress(context.TODO(), &ec2.RevokeSecurityGroupEgressInput{
								//DryRun: aws.Bool(true),
								GroupId: aws.String(*i.GroupId),
								SecurityGroupRuleIds: rules_egress,
							})
						
							if err != nil {
								fmt.Println("Error deleting egress rules:")
								fmt.Println(err)
								return
							}
						}
						*/
						...
						/*
						// filter for name begining "k8s-elb"
