...
func main() {
	...
	cmdCreate := &cobra.Command{
		...
		Run: func(cmd *cobra.Command, args []string) {
			...
			if (config.Cloud == "gcp") || (config.Cloud == "vsphere") {
...
func run_predelete(confCloud string, confName string, confNode string, confPath string) {
	...
	cmd := exec.Command("/usr/bin/ssh", "-oStrictHostKeyChecking=no", "-oStrictHostKeyChecking=no", "-i", "keys/id_rsa."+confCloud+"."+confName, "root@"+ip, `
